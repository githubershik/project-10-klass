...
def check_results(correct_answers):
    print(f"Ваше количество правильных ответов за данную сессию:{correct_answers}")
    ...
    if correct_answers > (top_result if record and record[0] is not None else 0):
        cursor.execute(f"UPDATE users SET top_result = {correct_answers} WHERE nickname='{login}'")
        conn.commit()
...
def register():
    ...
    if q == 1:
        ...
        cursor.execute(f"SELECT * FROM users WHERE nickname = '{login}' AND password = {password}")
        ...
        record = cursor.fetchall()
        if record == []:
            cursor.execute(f"INSERT INTO users (nickname,password) VALUES ('{login}', {password})")
            conn.commit()
            res = cursor.execute(f"SELECT * FROM users WHERE nickname = '{login}'")
            record = cursor.fetchall()
            if record != []:
